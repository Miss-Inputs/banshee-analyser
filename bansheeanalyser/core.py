...
def get_db(readonly=True):	
	banshee_db_path = expanduser('~/.config/banshee-1/banshee.db')
	...
	conn.create_function('does_uri_exist', 1, does_uri_exist)
	return conn
...
if __name__ == '__main__':
	print('secret test')
	#print(is_equal_number_insensitive('(cat', '(cats'))
	#print(is_equal_number_insensitive('cat and dogs', 'cats and dogs'))
	#print(p.singular_noun('cat'))
	#print(p.plural_noun('cats'))
	#print(ensure_singular('sheep and cats'))
	#print(query_first(get_db(), 'SELECT ensure_singular(?)', 'cats'))
	#print(get_artist_name(get_db(), 1))
	#print(banshee_is_running())
	#print(get_top_albums_by_average_rating(get_db()))
	#for album in get_top_albums_by_average_rating(get_db(), 30, 10):
	#	print(album)
	#for album in get_duplicate_album_ids(get_db()):
	#	if album['track_count'] > 0:
	#		print(album)
	#tracks = get_inconsistent_album_year(db)
	#print(len(tracks))
	#for track in tracks:
	#	print(track)
	#print(get_most_profilic_artists(get_db(), 10))
	print(get_track_by_id(get_db(), 13432))
	albums = get_albums_with_rating_aggregates(get_db(), 5)
	blah = sorted(albums, key=lambda album: album['average_rating'])[:20]
	for b in blah:
		print(b)